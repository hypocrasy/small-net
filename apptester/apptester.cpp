...
{
	initgraph(800, 600, EW_SHOWCONSOLE);//一个友好的用户界面（笑）
	setbkcolor(WHITE);
	cleardevice();
	...
	outtextxy(40, 40, _T("请输入ip地址："));
	outtextxy(40, 80, _T("请输入要传输的数据内容："));

	outtextxy(300, 40, _T("以下是已经发送的信息"));
	outtextxy(300, 300, _T("以下是收到的信息"));
	outtextxy(40, 400, _T("设备号为："));

	setfillcolor(LIGHTGRAY);
	fillrectangle(50, 100, 240, 150);
	settextstyle(15, 0, _T("微软黑体"));
	setbkmode(TRANSPARENT);
	outtextxy(100, 120, _T("我要发送文字！"));
	setfillcolor(LIGHTGRAY);
	fillrectangle(50, 160, 240, 210);
	settextstyle(15, 0, _T("微软黑体"));
	setbkmode(TRANSPARENT);
	outtextxy(100, 180, _T("我要发送图片！"));
	/*IMAGE img1;
	loadimage(&img1, _T("test.jpg"),90,90);
	//outtextxy(300, 320 + 18 * ipostionOffset, point);
	putimage(300, 60 + 18 * 0, &img1);*/
	...
	outtextxy(140, 400, strDevID.c_str());
